...
public class PostCommentController {
    ...
    // 댓글 작성
    @PostMapping
    public ResponseEntity<PostCommentResponse> create(@PathVariable Long postId, @Valid @RequestBody PostCommentCreateRequest request,@AuthenticationPrincipal Long memberId){
    ...
    public ResponseEntity<PostCommentResponse> update(
        ...
        PostCommentResponse response = postCommentService.update(memberId,commentId, request);
        return ResponseEntity.ok(response);
    }

    // 댓글 삭제
    @DeleteMapping("/{commentId}")
    public ResponseEntity<Void> delete(@PathVariable Long commentId,@AuthenticationPrincipal Long memberId) {
        postCommentService.delete(memberId,commentId);
